...
totalPrice (MkShoppingEntry _ p t) = p * fromIntegral t
...
buyOneMore (MkShoppingEntry n p t) = MkShoppingEntry n p (t + 1)
...
setName name (Person a _) = Person a name

-- setAge does likewise for age
setAge :: Int -> Person -> Person
setAge age (Person _ n) = Person age n
...
rgb Green = [0, 1, 0]
rgb Blue = [0, 0, 1]
rgb (Mix c1 c2) = mix (rgb c1) (rgb c2)
...
toList Empty = []
toList (Pair k v col) = (k, v) : toList col
...
toNat n
  | n < 0 = Nothing
  | otherwise = (Just . PlusOne . fromJust . toNat) (n - 1)
