...
mapMaybe2 f (Just v1) (Just v2) = Just $ f v1 v2
...
firstHalf s
  | even len = take (len `div` 2) s
  | otherwise = take (len `div` 2 + 1) s
  where
    len = length s
...
joinToLength n strings = (foldl' (++) [] . map (f n strings)) strings
  where
    f n l s = filter ((== n) . length) [s ++ str | str <- l]
...
sumRights = foldr (+) 0 . map (fromRight 0)
...
interpreter = f (0, 0)
  where
    f _ [] = []
    f (x, y) ("up" : xs) = f (x, y + 1) xs
    f (x, y) ("down" : xs) = f (x, y - 1) xs
    f (x, y) ("left" : xs) = f (x - 1, y) xs
    f (x, y) ("right" : xs) = f (x + 1, y) xs
    f (x, y) ("printX" : xs) = show x : f (x, y) xs
    f (x, y) ("printY" : xs) = show y : f (x, y) xs
