...
instance Ord Country where
  -- compare Finland Norway = LT -- implement me?
  -- compare Finland Switzerland = LT -- implement me?
  -- compare Norway Switzerland = LT -- implement me?

  (<=) Finland Norway = True -- and me?
  (<=) Finland Switzerland = True -- and me?
  (<=) Norway Switzerland = True -- and me?
  (<=) a b 
    | a == b = True
    | otherwise = False
  -- min a b
...
instance Eq Name where
  (==) (Name s1) (Name s2) = toLowerCase s1 == toLowerCase s2
    where
...
instance Price (Maybe Egg) where
  price Nothing = 0
  price (Just egg) = price egg

instance Price (Maybe Milk) where
  price Nothing = 0
  price (Just m) = price m

instance Price a => Price [a] where
  price [] = 0
  price xs = foldr cb 0 xs
    where
      cb ele acc = price ele + acc
...
simplify (RationalNumber n d) = RationalNumber (n `div` commonFactor) (d `div` commonFactor)
  where
...
instance Num RationalNumber where
  ...
  signum (RationalNumber n _d) 
    | n == 0 = 0
    | n < 0 = -1
    | otherwise = 1
  fromInteger x = RationalNumber x 1
  negate (RationalNumber n d) = RationalNumber (negate n) d
