...
isInCoordSet :: [Coord] -> Coord -> Bool
isInCoordSet cs c = Set.member c cSet
  where
    cSet = Set.fromList cs

prettyPrint :: Size -> [Coord] -> String
prettyPrint n cs = go 1 1 n
  where
    isInSet = isInCoordSet cs
    go row col n 
      | row > n = ""
      | col > n = '\n' : go (row + 1) 1 n
      | otherwise = if isInSet (row, col)
                    then 'Q' : go row (col + 1) n
                    else '.' : go row (col + 1) n
...
prettyPrint2 n cs = go 1 1 n
  where
    isInSet = isInCoordSet cs
    go row col n 
      | row > n = ""
      | col > n = '\n' : go (row + 1) 1 n
      | isInSet (row, col) = 'Q' : go row (col + 1) n
      | danger (row, col) cs = '#' : go row (col + 1) n
      | otherwise = '.' : go row (col + 1) n
...
fixFirst n ((i, j) : cs)
  | i > n || j > n = Nothing
  | danger (i, j) cs = fixFirst n $ (i, j + 1) : cs
  | otherwise = Just $ (i, j) : cs
--------------------------------------------------------------------------------
