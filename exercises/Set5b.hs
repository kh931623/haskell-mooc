...
treeMax Empty = 0
treeMax (Node v l r) = max v subMax
  where
    subMax = max (treeMax l) (treeMax r)
...
allValues _condition Empty = True
...
isOrdered (Node v l r) = gt v l && lt v r && isOrdered l && isOrdered r
  where
    gt val Empty = True
    gt val (Node v _ _) = val >= v
    lt val Empty = True
    lt val (Node v _ _) = val <= v
...
walk (StepL : xs) (Node _ l r) = walk xs l
walk (StepR : xs) (Node _ l r) = walk xs r
...
search val (Node v l r)
  | val == v = Just []
  | isNothing leftResult && isNothing rightResult = Nothing
  | isJust leftResult = Just (StepL : fromJust leftResult)
  | otherwise = Just (StepR : fromJust rightResult)
  where
    leftResult = search val l
    rightResult = search val r