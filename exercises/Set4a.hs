...
compareHead :: (Ord a) => [a] -> [a] -> Ordering
compareHead xs ys
  | lh > rh = LT
  | lh < rh = GT
  | otherwise = EQ
...
longest :: (Ord a) => [[a]] -> [a]
longest [] = []
longest xs = maximumBy compareLength xs
...
transfer from to amount bank
  ...
  | amount <= 0 = bank
  | bank Map.! from < amount = bank
  | otherwise = Map.adjust (+ amount) to . Map.adjust (\x -> x - amount) from $ bank
...
swap i j arr = arr // [(i, arr ! j), (j, temp)]
  where
    temp = arr ! i
...
maxIndex arr = fst $ foldr1 f (assocs arr)
  where
    f m n
      | snd n > snd m = n
      | otherwise = m