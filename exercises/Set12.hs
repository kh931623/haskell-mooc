...
instance Functor List where
  fmap f Empty = Empty
  fmap f (LNode v xs) = LNode (f v) (fmap f xs)
...
instance Functor TwoList where
  ...
  fmap f (TwoNode v1 v2 xs) = TwoNode (f v1) (f v2) (fmap f xs)
...
instance Foldable List where
  foldr f z Empty = z 
  foldr f z (LNode v xs) = f v (foldr f z xs)
...
instance Foldable TwoList where
  ...
  foldr f z (TwoNode v1 v2 xs) = f v1 (f v2 (foldr f z xs))
