...
step HasFlour AddSugar = HasFlourAndSugar
step HasSugar AddFlour = HasFlourAndSugar
step HasFlourAndSugar Mix = Mixed
step Mixed Bake = Finished
step Finished _e = Finished
step _s _e = Error
...
average (x :| []) = x
average (x :| xs) = f 0 0 (x:xs)
  where
    f n acc [] = acc / n
    f n acc (x:xs) = f (n + 1) (acc + x) xs
...
instance Semigroup Time where
  (<>) (Time d1) (Time d2) = Time (d1 + d2)

instance Semigroup Velocity where
  (<>) (Velocity d1) (Velocity d2) = Velocity (d1 + d2)
...
instance Operation2 Add2 where
  ...
  display (Add2 i j) = show i ++ "+" ++ show j

instance Operation2 Subtract2 where
  compute2 (Subtract2 i j) = i-j
  display (Subtract2 i j) = show i ++ "-" ++ show j

instance Operation2 Multiply2 where
  compute2 (Multiply2 i j) = i * j
  display (Multiply2 i j) = show i ++ "*" ++ show j
...
passwordAllowed str (ContainsSome xs) = any (`elem` str) xs
passwordAllowed str (DoesNotContain xs) = all (`notElem` str) xs
...
evaluate (Add a1 a2) = evaluate a1 + evaluate a2
evaluate (Sub a1 a2) = evaluate a1 - evaluate a2
evaluate (Mul a1 a2) = evaluate a1 * evaluate a2

render :: Arithmetic -> String
render (Literal v) = show v
render (Add a1 a2) = "(" ++ render a1 ++ "+" ++ render a2 ++ ")"
render (Mul a1 a2) = "(" ++ render a1 ++ "*" ++ render a2 ++ ")"
render (Sub a1 a2) = "(" ++ render a1 ++ "-" ++ render a2 ++ ")"
